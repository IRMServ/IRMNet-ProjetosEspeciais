...
namespace ProjetosEspeciais\Controller;
...
class HelpDeskController extends AbstractActionController {
    ...
    public function indicadoresAction() {
        ...
        $data = $date->sub(new \DateInterval('P1M'));
       
       
        
        $chamados_abertos_query = $em->createQuery("SELECT '*' FROM Helpdesk\Entity\Chamado Chamado where Chamado.setor_destino_fk={$setor} and Chamado.datafim IS NULL and Chamado.datainicio like '%2013-03%'");
        //$chamados_abertos_query = $em->createQuery("SELECT '*' FROM Helpdesk\Entity\Chamado Chamado where Chamado.setor_destino_fk={$setor} and Chamado.datafim IS NULL and Chamado.datainicio like '%{$data->format('Y-m')}%'");
        
        $chamados_abertos = $chamados_abertos_query->getResult();
        //exit("SELECT '*' FROM Helpdesk\Entity\Chamado Chamado where Chamado.setor_destino_fk={$setor} and Chamado.datafim IS NULL and Chamado.datainicio like {$data->format('Y-m-d')}");
        //echo count($chamados_abertos);
        ...
        return new ViewModel(array('media'=>number_format($media,2,'.',','),'chamados'=>$chamados_nn,'total_chamados'=>count($chamados),'chamados_na'=>count($chamados_nn),'chamados_abertos'=>count($chamados_abertos)));
    }
}